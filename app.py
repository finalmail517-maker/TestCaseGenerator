...
from cucumber_generator.cucumber_generator import CucumberGenerator
...
if "cucumber_generator" not in st.session_state:
    st.session_state.cucumber_generator = CucumberGenerator(
...
if "last_cucumber_file" not in st.session_state:
    st.session_state.last_cucumber_file = None
...
def display_sidebar():
    with st.sidebar:
        ...
        unit = st.checkbox("Unit tests", value=True, key="cb_unit")
        functional = st.checkbox("Functional tests", value=False, key="cb_functional")

        st.divider()

        st.subheader("Other")
        lld = st.checkbox("From LLD doc (.docx)", value=False, key="cb_lld")
        cucumber = st.checkbox("Cucumber / BDD", value=False, key="cb_cucumber")

        selected_test_types = []
        if unit:
            selected_test_types.append("Unit Test")
        if functional:
            selected_test_types.append("Functional Test")
        if lld:
            selected_test_types.append("LLD")
        if cucumber:
            selected_test_types.append("Cucumber")

        st.session_state.selected_test_types = selected_test_types

        if lld:
            st.info("📄 LLD mode: upload a .docx file in the chat area.")
        elif cucumber:
            st.info("🥒 Cucumber mode: paste code, upload a file, or paste a GitHub URL.")
        ...
        if history_dir.exists():
            ...
            if chat_files:
                ...
                for chat_file in chat_files[:10]:
                    ...
                    display_name = parts[0] if len(parts) >= 3 else name
    ...
    return selected_test_types

# ---- Unified Chat UI ------------------------------------------------------------
def display_chat():
    st.subheader("AI Test Case Generator")

    selected_types = st.session_state.get("selected_test_types", ["Unit Test"])
    is_lld_mode = "LLD" in selected_types
    is_cucumber_mode = "Cucumber" in selected_types

    with st.container():

        # ── Input area changes based on mode ─────────────────
        if is_cucumber_mode:
            st.caption("🥒 **Cucumber mode** — paste a GitHub URL, paste code directly, or upload a code file below.")
            user_input = st.chat_input("Paste GitHub URL or paste your code here...")

            uploaded_code_file = st.file_uploader(
                "Upload code file",
                type=["py", "java", "js", "ts", "cs", "rb", "go", "kt", "cpp", "c"],
                key="cucumber_file_uploader",
                label_visibility="collapsed",
            )

            # Handle uploaded code file for cucumber
            if uploaded_code_file is not None:
                file_key = f"cuc_{uploaded_code_file.name}_{uploaded_code_file.size}"
                if st.session_state.get("last_cucumber_file") != file_key:
                    st.session_state.last_cucumber_file = file_key
                    code_content = uploaded_code_file.read().decode("utf-8", errors="ignore")
                    st.session_state.uploaded_files[uploaded_code_file.name] = code_content
                    st.success(f"✅ Loaded: {uploaded_code_file.name}")

        elif is_lld_mode:
            st.caption("📄 **LLD mode** — upload your .docx file below to auto-generate tests.")
            user_input = st.chat_input("Or type a message...")

        else:
            user_input = st.chat_input("Paste GitHub URL, type 'generate', or ask a question...")

        # ── LLD file uploader (always shown in LLD mode) ─────
        lld_file = st.file_uploader(
            "📄 Upload LLD Word Document (.docx)",
            type=["docx"],
            key=f"lld_uploader_{st.session_state.lld_uploader_key}",
            label_visibility="collapsed",
        ) if is_lld_mode else None

        # ── LLD processing (unchanged from your original) ────
        if lld_file is not None:
            ...
            with st.chat_message("assistant"):
                with st.spinner("Reading LLD and generating test cases..."):
                    start = time.time()
                    lld_h = st.session_state.lld_handler
                    test_types_for_lld = ["Unit Test", "Functional Test"]
                    tests = lld_h.process_lld_file(file_bytes, filename, test_types_for_lld)
                    ...
                    with d1:
                        with open(csv_file, "rb") as f:
                            st.download_button("📥 Download CSV", data=f,
                                file_name=f"lld_tests_{datetime.now():%Y%m%d_%H%M%S}.csv", mime="text/csv")
                    with d2:
                        with open(report_file, "rb") as f:
                            st.download_button("📥 Download Report", data=f,
                                file_name=f"lld_report_{datetime.now():%Y%m%d_%H%M%S}.txt", mime="text/plain")

                    for ttype in test_types_for_lld:
                        lst = tests.get(ttype, [])
                        if lst:
    ...
    if user_input:
        ...
        # ── CUCUMBER MODE ─────────────────────────────────────
        cucumber_keywords = [
            "generate cucumber", "generate bdd", "generate feature",
            "generate step", "cucumber", "bdd"
        ]
        is_cucumber_request = (
            is_cucumber_mode or
            any(k in sanitized.lower() for k in cucumber_keywords)
        )

        if is_cucumber_request:
            # Build code from: uploaded files, pasted code, or GitHub URL
            combined_code = ""
            ...
            # From pasted code/message (if long enough to be code)
            if not combined_code and len(sanitized) > 80:
                combined_code = sanitized

            if not combined_code:
                bot = (
                    "Please provide code in one of these ways:\n"
                    "- Upload a code file using the uploader above\n"
                    "- Paste your code directly in the chat\n"
            ...
            with st.chat_message("assistant"):
                with st.spinner("🥒 Generating Cucumber BDD scripts..."):
                    result = st.session_state.cucumber_generator.generate(combined_code)

                    st.subheader("📄 Feature File")
                    st.code(result.get("feature_file", ""), language="gherkin")

                    st.subheader("🔧 Step Definitions")
                    st.code(result.get("step_definitions", ""), language="python")

                    st.subheader("🚀 Test Runner")
                    st.code(result.get("runner_class", ""), language="python")

                    bot = "✅ Cucumber BDD scripts generated successfully."
                    st.session_state.chat_history.append(
                        {"role": "assistant", "content": bot, "timestamp": datetime.now().isoformat()}
                    )
                    auto_save_chat()
        ...
        if "generate" in sanitized.lower() and st.session_state.uploaded_files:
            test_types = [t for t in selected_types if t in ["Unit Test", "Functional Test"]]
            if not test_types:
            ...
            with st.chat_message("assistant"):
                with st.spinner("Generating tests from uploaded files..."):
                    ...
                    with d1:
                        with open(csv_file, "rb") as f:
                            st.download_button("📥 Download CSV", data=f,
                                file_name=f"tests_{datetime.now():%Y%m%d_%H%M%S}.csv", mime="text/csv")
                    with d2:
                        with open(report_file, "rb") as f:
                            st.download_button("📥 Download Report", data=f,
                                file_name=f"report_{datetime.now():%Y%m%d_%H%M%S}.txt", mime="text/plain")
        ...
        if st.session_state.pending_git:
            ...
            if pend["stage"] == "ask_branch":
                ...
                bot = f"Branch `{branch}` selected. Cloning and generating test cases..."
                st.session_state.chat_history.append(
                    {"role": "assistant", "content": bot, "timestamp": datetime.now().isoformat()}
                )
                with st.chat_message("assistant"):
        ...
        if pend["stage"] == "processing":
            with st.spinner("Cloning & analysing repository…"):
                try:
                    ...
                    # If cucumber mode, generate cucumber scripts
                    if is_cucumber_mode:
                        combined_code = ""
                        parser = CodeParser()
                        ...
                        with st.chat_message("assistant"):
                            with st.spinner("🥒 Generating Cucumber BDD scripts from repo..."):
                                result = st.session_state.cucumber_generator.generate(combined_code)
                                st.subheader("📄 Feature File")
                                st.code(result.get("feature_file", ""), language="gherkin")
                                st.subheader("🔧 Step Definitions")
                                ...
                                st.subheader("🚀 Test Runner")
                                st.code(result.get("runner_class", ""), language="python")
                                bot = "✅ Cucumber BDD scripts generated from repository."
                                st.session_state.chat_history.append(
                                    {"role": "assistant", "content": bot, "timestamp": datetime.now().isoformat()}
                                )
                        st.session_state.pending_git = None
                    ...
                    # Normal unit/functional test generation from repo
                    parser = CodeParser()
                    parsed = {}
                    prog = st.progress(0)
                    ...
                    logger.error(f"Git processing error: {e}", exc_info=True)
                    st.session_state.pending_git = None
